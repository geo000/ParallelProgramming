...
Result segment(int ny, int nx, const float* data) {
    ...
    for (int y = 0; y < ny+1; y++)
    {
        ...
        {
            interm_data_vector[x + (nx+1) * y] = double4_0;
        }
    }
    for (int y1 = 0; y1 < ny; y1++) {
        for (int x1 = 0; x1 < nx; x1++) {
            interm_data_vector[(x1+1) + (nx+1) * (y1+1)] = vectorized_data[x1 + nx * y1] + interm_data_vector[x1 + (nx+1) * (y1+1)] 
                                                         + interm_data_vector[(x1+1) + (nx+1) * y1] - interm_data_vector[x1 + (nx+1) * y1];      
        }
    }

    double4_t all_colors_vector = interm_data_vector[nx + (nx+1) * ny];
    #pragma omp parallel
    {
            ...
            for (int width = 1; width <= nx; width++)
            {
                ...
                for (int y0 = 0; y0 <= ny - height; y0++)
                {
                    for (int x0 = 0; x0 <= nx - width; x0++)
                    {
                        ...
                        int x1 = x0 + width;
                        double best = 0;
                        double4_t sum_of_colors_X = interm_data_vector[x1 + (nx+1) * y1] - interm_data_vector[x0 + (nx+1) * y1] 
                                        - interm_data_vector[x1 + (nx+1) * y0] + interm_data_vector[x0 + (nx+1) * y0];
                        double4_t sum_of_colors_Y = all_colors_vector - sum_of_colors_X;
                        double4_t best4 = sum_of_colors_X * sum_of_colors_X * pixel_x + sum_of_colors_Y * sum_of_colors_Y * pixel_y;
